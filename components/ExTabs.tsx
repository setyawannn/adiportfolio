  ...
  return (
    <div>
      <Tabs selectedIndex={activeTab} onSelect={handleActiveSelect}>
          ...
          {DataTabList.map((data, index) => {
            return (
              <Tab
                key={index}
                className={`
                  "h-8 text-lg pr-4 text-secondary py-2 font-bold outline-none cursor-pointer" ${
                    activeTab == index ? "border-b-[3px] border-secondary" : ""
                  }
                  `}
              >
                {data.name}
            ...
            <TabPanel>
              <div key={index} className="mt-6">
                ...
                <div className="flex flex-row gap-4">
                  <div className="w-1/3">
                    <h6 className="text-base font-bold ">{data[index].date}</h6>
                  </div>
                  <div className="w-2/3">
        ...
        {/* <TabPanel>Ini dua guys</TabPanel> */}
      </Tabs>
    </div>
  );
}
